...
const useTime = () => {
  ...
  const getShiftHours = (
    ...
    setTotalHours(
      parseInt(hourEnd) + minsEnd - (parseInt(hourStart) + minsStart) - deduct
    );

    if (parseInt(hourStart) + minsStart > parseInt(hourEnd) + minsEnd) {
      setTotalHours(
        24 - (parseInt(hourStart) + minsStart) + (parseInt(hourEnd) + minsEnd)
      );
      if (
        24 - (parseInt(hourStart) + minsStart) + (parseInt(hourEnd) + minsEnd) >
        12
  ...
  const deductUp = () => {
    setTime({ ...time, deductBreak: parseInt(time.deductBreak) + 15 });

    if (parseInt(time.deductBreak) === 45) {
      setTime({ ...time, deductBreak: "00" });
    }
  };
  ...
  const deductDown = () => {
    setTime({
      ...time,
      deductBreak: parseInt(time.deductBreak) - 15
    });

    if (parseInt(time.deductBreak) === 0) {
      setTime({ ...time, deductBreak: "45" });
    }

    if (parseInt(time.deductBreak) === 15) {
      setTime({ ...time, deductBreak: "00" });
    }
  };
  ...
  const hoursStartUp = () => {
    setTime({
      ...time,
      hourStart:
        parseInt(time.hourStart) < 9
          ? `0${parseInt(time.hourStart) + 1}`
          : parseInt(time.hourStart) + 1
    });
  ...
  const hoursEndUp = () => {
    setTime({
      ...time,
      hourEnd:
        parseInt(time.hourEnd) < 9
          ? `0${parseInt(time.hourEnd) + 1}`
          : parseInt(time.hourEnd) + 1
    });
