...
const EditShift = ({ match }) => {
	...
	useEffect(
		() => {
			setTotalPay(parseFloat(Math.round(workShift.payRate * totalHours * 100) / 100).toFixed(2));
		},
		[ totalHours, time ]
	);
	...
	const fetchShift = async () => {
		...
		try {
			...
			setTimeout(() => {
				...
				const { shift_start, shift_end, name, date } = data.workShift;
	...
	const handleSubmit = (event) => {
		...
		if (totalHours === 0) {
			setFormErrors({ ...formErrors, totalHours: 'Total hours must not be 0!' });
		} else {
			setFormErrors({});
		}
	};
			...
			<div className="shift-info">
				<div className="shift-hours">
					<span className="shift-hours__title">
						Total Hours:{' '}
						<input
							className={`${formErrors.totalHours ? 'invalid-inputs' : 'shift-hours__count'}`}
							name="shift_hours"
							value={totalHours}
							readOnly
						/>
					...
					<div className="errors-container">
						{formErrors.totalHours && <span className="help inavild">{formErrors.totalHours}</span>}
					</div>
				</div>
	...
	const editForm = (
		<form onSubmit={handleSubmit} className="edit-shift__form">
			...
			<div className="form-btns">
				<button type="submit" className="edit-shift__form--submit form-submit__btn">
					Edit Shift
				</button>
				<button className="shift-delete__btn form-submit__btn" onClick={deleteShift}>
					Delete Shift
				</button>
			</div>
		</form>
